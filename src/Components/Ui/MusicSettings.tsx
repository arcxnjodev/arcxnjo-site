...
export const MusicSettings = () => {
  ...
  const uploadFile = async (file: File) => {
    ...
    if (!allowedTypes.includes(file.type)) throw new Error("Invalid file type. Allowed: MP3, WAV, OGG, WEBM.");
    if (file.size > 25 * 1024 * 1024) throw new Error("File is too large. Maximum size is 25MB.");
  ...
  const { values, handleSubmit, handleChange, setValues } = useFormik<MusicValues>({
    initialValues: { musicTitle: "", musicArtist: "", musicUrl: "" },
      ...
      try {
        ...
        await axios.put(
          `${API_URL}/api/profile/music`,
          {
            ...
            musicArtist: values.musicArtist.trim(),
    ...
    const fetchMusic = async () => {
      try {
        ...
        setValues({
          ...
          musicArtist: response.data.music_artist || "",
  ...
  const handleMusicUpload = async (file: File | undefined) => {
    ...
    try {
      ...
      setMessage("");
      if (!file.type.startsWith("audio/")) throw new Error("Music file must be audio.");
  ...
  const clearMusic = () => setValues({ musicTitle: "", musicArtist: "", musicUrl: "" });
    ...
    <div className="space-y-6">
      <div className="relative overflow-hidden rounded-3xl border border-white/10 bg-gradient-to-br from-purple-600/20 via-black/30 to-pink-600/10 p-5 md:p-6">
        ...
        <div className="relative">
          ...
          <h3 className="text-2xl font-black text-white">Música do perfil</h3>
          <p className="mt-2 max-w-2xl text-sm leading-relaxed text-white/50">
            Coloque uma música para tocar quando alguém entrar no seu perfil. A letra é buscada automaticamente pelo título e artista.
          </p>
        </div>
      </div>

      {message && (
        <div className={`rounded-2xl border px-4 py-3 text-sm font-semibold ${message.includes("✅") ? "border-green-400/20 bg-green-500/10 text-green-200" : "border-red-400/20 bg-red-500/10 text-red-200"}`}>
          {message}
        </div>
      )}
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <section className="rounded-3xl border border-white/10 bg-black/25 p-5">
          <div className="mb-5 flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-white/40">Nome da música, artista e link do arquivo.</p>
            </div>
          </div>

          <div className="grid gap-5 xl:grid-cols-2">
            <div>
              <label className="mb-2 block text-sm font-semibold text-white/85">Music Title</label>
              <div className="relative">
              ...
              <p className="mt-2 text-xs text-white/35">{values.musicTitle.length}/60</p>
            </div>

            <div>
              <label className="mb-2 block text-sm font-semibold text-white/85">Artist</label>
              <div className="relative">
                <FaMicrophone className="absolute left-4 top-1/2 -translate-y-1/2 text-white/30" />
                <input
                  type="text"
                  name="musicArtist"
                  placeholder="Artist name"
                  className={inputClass}
                  value={values.musicArtist}
                  onChange={handleChange}
                  maxLength={60}
                />
              </div>
        ...
        <section className="rounded-3xl border border-white/10 bg-black/25 p-5">
          <div className="mb-5">
            ...
            <p className="mt-1 text-sm text-white/40">Allowed formats: MP3, WAV, OGG, WEBM. Max 25MB.</p>
          </div>
          <label className="flex cursor-pointer items-center justify-center gap-2 rounded-2xl border border-dashed border-white/15 bg-white/[0.035] px-4 py-5 text-sm font-semibold text-white/60 transition hover:border-purple-400/35 hover:bg-purple-500/10 hover:text-white">
            ...
            <input type="file" accept="audio/mpeg,audio/wav,audio/ogg,audio/webm" className="hidden" onChange={(e) => handleMusicUpload(e.target.files?.[0])} />
          </label>
        </section>
            ...
            <div className="mb-4 flex items-center justify-between gap-4">
              <div>
                ...
                <p className="mt-1 text-sm text-white/40">
                  {values.musicTitle || "Profile music"}{values.musicArtist ? ` — ${values.musicArtist}` : ""}
                </p>
              </div>
              <button
                type="button"
          ...
          <div className="flex flex-col justify-between gap-4 md:flex-row md:items-center">
            <div>
              <p className="flex items-center gap-2 text-sm font-bold text-white">
                <FaCircleCheck className="text-green-300" />
                Ready to publish
              </p>
              <p className="mt-1 text-xs text-white/40">Music: {values.musicUrl ? "set" : "empty"}</p>
            </div>
            <button
              type="submit"
